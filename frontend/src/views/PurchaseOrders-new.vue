                ...
                <table class="order-items-table">
                  ...
                  <tbody>
                    <tr v-for="(item, idx) in newOrder.items" :key="idx">
                      ...
                      <td>
                        <input type="text" v-model="item.unit" :disabled="!!item.product_id" />
                      </td>
                      <td>
                  ...
                  <tbody>
                    <tr v-for="(item, idx) in selectedOrder.items" :key="idx">
                      <td>
                        <div v-if="isViewMode">
                          <div>
                            ...
                            <div style="color: #666; font-size: 12px; margin-top: 2px;">{{ item.description }}</div>
                          </div>
                        </div>
                      ...
                      <td>
                        ...
                        <input v-else type="text" v-model="item.unit" :disabled="!!item.product_id" />
                      </td>
                      <td>
                        <div v-if="isViewMode">
      ...
      try {
        ...
        const orderData = {
          ...
          items: this.newOrder.items.map(item => ({
            ...
            quantity: parseFloat(item.quantity) || 0,
            unit_price: parseFloat(item.unit_price) || 0,
            description: item.description || (item.product_name ? `${item.product_name} (${item.product_sku})` : '')
          })),
    ...
    viewOrder(order) {
      ...
      if (this.selectedOrder.items) {
        this.selectedOrder.items.forEach(item => {
          ...
          // 设置搜索字段
          item.searchTerm = item.product_name ? (item.product_name + ' - ' + item.product_sku) : ''
          item.filteredProducts = [...this.allProducts]
        })
      }
